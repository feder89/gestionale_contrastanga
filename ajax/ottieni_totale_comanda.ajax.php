<?php
  ...
  if(isset($_POST['tavolo']) && is_numeric($_POST['tavolo']) && isset($_POST['indice']) && is_numeric($_POST['indice']) && $_POST['indice']>=1 && isset($_POST['data']) && !empty($_POST['data'])){
    ...
    $query_ordini = "SELECT * 
      ...
      if($row_com = mysqli_fetch_assoc($result)){
          ...
          if($coperti == 0){
              $comanda='#error#La comanda non ha nessun coperto';
          }
          else {
      ...
      /* error */
      $comanda='#error#Errore durante l\'acquisizione dei dati';
    }
    ...
    /* error */
    $comanda='#error#Parametri non riconosciuti';
  }
