        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            /*if(isset($quantita_rimanente) && $quantita_rimanente){
                ...
                if($date <=0){
                    ...
                    $query_port = "SELECT * FROM Quantit�PiattiSerata WHERE serata='$date' AND piatto = '".$row['nome_portata']."'";
                    if( ($res2 = esegui_query($link, $query_port))){ 
                        $row2 = mysqli_fetch_assoc($res2);
                        $portate[]=array('nome_portata' => $row['nome_portata'], 'categoria' => $row['categoria'], 'prezzo_finale' => $row['prezzo_finale'], 'quantita_rimanente' => $row2['quantit�']);
                    }
                    else{
                        /* error *//*
                        $portate[]=array('error' => '#error#Errore durante l\'acquisizione dei dati');
