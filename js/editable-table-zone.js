    ...
    return {
        ...
        init: function () {
            ...
            function modifyRow(oTable, nRow) {
                var aData = oTable.fnGetData(nRow);
                var jqTds = $('>td', nRow);
                jqTds[0].innerHTML = '<input type="text" maxlength="50" class="form-control small" value="' + aData[0] + '" disabled>';
                jqTds[1].innerHTML = '<input disabled type="text" class="form-control small" value="' + aData[1] + '">';
                jqTds[2].innerHTML = '<input disabled type="text" class="form-control small" value="' + aData[2] + '">';
                jqTds[3].innerHTML = 'Aggiornamento non consentito';
                jqTds[4].innerHTML = '<a class="cancel" href="">Annulla</a>';
            }
            ...
            $('#editable-sample a.delete').live('click', function (e) {
                ...
                var zona = tr.children('td:nth-child(1)');  

                if (confirm("Sei sicuro di voler cancellare la zona \""+zona.text()+"\"?") == false) {
                    return false;
                }
            
                $.ajax({
                    type: 'POST',
                    url: "ajax/gestisci_zone.ajax.php",
                    dataType: "text",
                    timeout: 20000,
                    data : {
                        operazione: 'cancella',
            ...
            $('#editable-sample a.edit').live('click', function (e) {
                ...
                if(nEditing !== null && nEditing != nRow && nEditing.classList.contains('nuovariga')){
                    ...
                    var zona = tr.children('td:first-child').find('input');

                    if(zona.val() == ''){
                        zona.addClass('input-error');
