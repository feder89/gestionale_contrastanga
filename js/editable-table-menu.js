    ...
    return {
        ...
        init: function () {
            ...
            function editRow(oTable, nRow, new_row) {
                var aData = oTable.fnGetData(nRow);
                var jqTds = $('>td', nRow);
                jqTds[0].innerHTML = '<input type="text" maxlength="50" class="form-control small" value="' + aData[0] + '">';
                jqTds[1].innerHTML = '<select><option value="1">Si</option><option value="0" selected>No</option></select>';
            ...
            function modifyRow(oTable, nRow) {
                var aData = oTable.fnGetData(nRow);
                var jqTds = $('>td', nRow);
                jqTds[0].innerHTML = '<input disabled type="text" maxlength="50" class="form-control small" value="' + aData[0] + '">';
                jqTds[1].innerHTML = '<select><option value="1" '+( aData[1] == 'Si' ? 'selected': '')+'>Si</option><option value="0" '+( aData[1] == 'No' ? 'selected': '')+'>No</option></select>';
                jqTds[2].innerHTML = '<input step="any" min="0.00" max="99.99" type="number" class="form-control small num"  value="' + parseInt(aData[2]) + '">';
                jqTds[3].innerHTML = '<input disabled type="text" class="form-control small" value="' + aData[3] + '">';
                jqTds[4].innerHTML = '<input disabled type="text" class="form-control small" value="' + aData[4] + '">';
                jqTds[5].innerHTML = '<a class="edit" href="">Aggiorna</a>'; /* se cambiamo Aggiorna qui, cambia anche dentro modal-serate.js */
                jqTds[6].innerHTML = '<a class="cancel" href="">Annulla</a>';
            }
            ...
            function saveRow(oTable, nRow) {
                ...
                oTable.fnUpdate(select.val() == 1 ? 'Si' : 'No', nRow, 1, false);
                oTable.fnUpdate((jqInputs[1].value && select.val()==1) ? jqInputs[1].value+' €' : '-', nRow, 2, false);
            ...
            $('#editable-sample a.delete').live('click', function (e) {
                ...
                if (confirm("Sei sicuro di voler cancellare il menù \""+nome.text()+"\"?") == false) {
                    return;
                }
                //console.log(nome);              
                $.ajax({
                        type: 'POST',
                        url: "ajax/gestisci_menu.ajax.php",
                        //dataType: "text",
                        timeout: 20000,
                        data : {
                            operazione: 'cancella',
            ...
            $('#editable-sample a.edit').live('click', function (e) {
                ...
                if(nEditing !== null && nEditing != nRow && nEditing.classList.contains('nuovariga')){
                    ...
                    var fisso = tr.children('td:nth-child(2)').find('select');
                    var costo = tr.children('td:nth-child(3)').find('input');
                   	...
                   	if(nome.val() == ''){
						...
						nome.keyup(function(){
                   		...
                   		return false;
                   	} else if(fisso.val() == 1 && ( costo.val() == '' || costo.val()<=0 || costo.val()>99.99) ){
