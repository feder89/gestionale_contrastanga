...
if(!($res=esegui_query($link,$query))){
	...
	while($row=mysqli_fetch_assoc($res)){
		...
		if(!($res1=esegui_query($link,$query_ordini))){
            ...
            }elseif($pagata == 0 &&  $attiva == 1){
            	$tot_serata_da_riscuotere +=$totale_fin;
            }
...
$tot_serata_da_riscuotere = number_format($tot_serata_da_riscuotere, 2, '.', ' ');
$exporter->addRow(array("Totale incasso serata:", "€ ".$tot_serata));
$exporter->addRow(array("Totale non incassato della serata:", "€ ".$tot_serata_da_riscuotere));
$exporter->addRow(array("Totale non incassato della serata:", "€ ".$tot_serata_non_incassato)); 

$exporter->addRow("");
$exporter->addRow("");*/
$exporter->addRow(array("Riepilogo Portate Vendute"));

$query_ordini_serata="SELECT o.portata, p.categoria, SUM(o.quantita) AS num_portate, p.prezzo_finale
					  FROM Ordini o
            		  INNER JOIN Portata p
            		  ON p.nome_portata=o.portata
            		  WHERE serata='$date'
...
if(!($res=esegui_query($link,$query_ordini_serata))){
    $exporter->addRow("errore");
}
else {
	$exporter->addRow(array("Portata", "Categoria", "Quantità", "Prezzo"));
	while(($row = mysqli_fetch_assoc($res))){
		$exporter->addRow(array($row['portata'],ucfirst($row['categoria']),$row['num_portate'],"€ ".number_format($row['prezzo_finale'], 2, '.', ' ')));
	}
}
